...
(deftest generates-extended-invoice-headers-test
  (let [invoice (receipt {:infoFactura
...
(deftest generates-subsidy-fields-test
  (let [invoice (receipt {:infoFactura {:totalSubsidio "0.00"}
...
(deftest generates-items-test
  (let [invoice (receipt {:detalles [{:descripcion "Prueba"
                                      :cantidad "10.00"
                                      :precioUnitario "1.00"
                                      :unidadMedida "Kilos"
...
(deftest generates-additional-info-test
  (let [invoice (receipt {:infoAdicional [{:nombre "nombre"
                                           :valor "valor"}]})]
    (are [x y] (= x y)
      (map
       (fn [i]
...
(deftest generates-withholdings-test
  (let [invoice (receipt {:retenciones [{:codigo "0"
                                         :codigoPorcentaje "0"
                                         :tarifa "0.01"
                                         :valor "0.10"}]})]
    (are [x y] (= x y)
